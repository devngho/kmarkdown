...
class Readme: StringSpec({
    "README.md" {
        val document = markdown(GFMFlavor) {
            +block {
                +(text("English") + text(" | ") + link("한국어", "README_ko.md"))
            }

            +heading(2, "kmarkdown") {
                +block {
                    +("kmarkdown is a" _ bold { +"well-tested" } _ "and" _ bold { +"pure-Kotlin" } _ "Markdown library.")
                }
                +block {
            ...
            +heading(3, "Changelog") {
                +list(List.ListStyle.UNORDERED) {
                    ...
                    list(List.ListStyle.UNORDERED) {
                        ...
                        item { +"TextStyle is just a MarkdownElement now." }
                    }
            ...
            +heading(3, "Usage") {
                ...
                +heading(4, "Raw") {
                    +blockquote {
                        +block {
                            + bold { +"It's not recommended to use raw elements." }
                        }
                        +block {
    ...
    "README_ko.md" {
        val document = markdown(GFMFlavor) {
            +block {
                +(link("English", "README.md") _ " | " _ text("한국어"))
            }

            +heading(2, "kmarkdown") {
                +block {
                    +("kmarkdown은" _ bold { +"잘 테스트된" } + "," _ bold { +"순수한 코틀린" } _ "마크다운 라이브러리입니다.")
                }
                +block {
            ...
            +heading(3, "변경 내역") {
                +list(List.ListStyle.UNORDERED) {
                    ...
                    list(List.ListStyle.UNORDERED) {
                        ...
                        item { +"TextStyle는 이제 일반 MarkdownElement입니다." }
                    }
            ...
            +heading(3, "사용") {
                ...
                +heading(4, "Raw") {
                    +blockquote {
                        +block {
                            +bold { +"Raw 요소를 사용하는 것은 권장하지 않습니다." }
                        }
                        +block {
